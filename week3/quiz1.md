...
- 답: 1은 공통 부모로 갖는 커밋을 생성하여 브랜치 히스토리를 보존하고, 2는 여러 커밋을 하나로 합친 새로운 커밋을 새로 만들어 브랜치 히스토리를 깔끔하게 유지할 수 있으며 3은 다른 브랜치에 있는 모든 커밋을 main의 최근 커밋을 부모로 하여 추가한다.
...
- 답: 
1. 현재 작업중인 브랜치는 `develop`이다.
2. 최신 commit ID는 a1s2d3f이다. 
3. 해당 커밋에서 readme를 추가하였다.
...
- 답: revert. 협업 상황에서 reset을 이용하면 커밋을 삭제하면 충돌 가능성이 존재한다.
